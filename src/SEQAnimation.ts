...
export class SEQAnimation {
  ...
  readKey() {
    ...
    if ((code & 0xe0) > 0) {
      ...
      const shortFrameCount = code & 0x1f;
      f = shortFrameCount === 0x1f ? 0x20 + r.u8() : 1 + shortFrameCount;
    } else {
      // number of frames, half word case

      const shortFrameCount = code & 0x3;
      f = shortFrameCount === 0x3 ? 4 + r.u8() : 1 + shortFrameCount;
